...
Minimal Hono HTTP API served by `@hono/node-server`. The build runs
`npm install --omit=dev` so only production dependencies land in the image.
...
```bash
docker run --rm -p 8080:8080 node-hono-sample
```
